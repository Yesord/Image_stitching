...
from project.utils.myblender import Blender
...
def execute(index1, index2, prevH):
    ...
    cv2.imwrite('outputs/l' +  '/custom/warped_' + str(index2) +  '.png', warpedImage)
...
if __name__ == "__main__":
    ...
    images = [cv2.imread(os.path.join(image_path, file)) for file in os.listdir(image_path) if file.endswith('.jpg')]
    os.makedirs('outputs/l' + '/custom/', exist_ok = True)
    ...
    finalImg =  cv2.imread('outputs/l' +  '/custom/'  + 'warped_' + str(0) + '.png')
    length = len(images)
    for index in range(1, length):
        print('blending', index)
        img2 = cv2.imread('outputs/l'  + '/custom/' + 'warped_' + str(index) + '.png')
        ...
        cv2.imwrite('outputs/l' + '/custom/' 'FINALBLENDED.png', finalImg)
